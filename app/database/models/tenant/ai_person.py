from sqlalchemy import Column, Integer, String, Boolean, DateTime, Date, Text, UUID, ForeignKey, Float  # ADD Float
...
class AIPerson(Base, TimestampMixin, SchemaConfigMixin):
    ...
    ai_feedback = relationship("AIFeedback", back_populates="ai_person", cascade="all, delete-orphan")  # ADD
    ai_decision_audits = relationship("DecisionAudit", back_populates="ai_person", cascade="all, delete-orphan")  # ADD
    ai_recommendation_logs = relationship("AIRecommendationLog", back_populates="ai_person", cascade="all, delete-orphan")  # ADD
    ai_suppression_logs = relationship("SuppressionLog", back_populates="ai_person", cascade="all, delete-orphan")  # ADD
    family = relationship("AIFam", back_populates="family_members", foreign_keys=[fam_id], viewonly=True)

...
from app.core.api_key_auth import get_current_user, User
from app.database.tenant_management import get_tenant_by_id
from app.core.report_management import ReportManagement
...
async def delete_followup_report(
    ...
    if not current_user.has_permission("delete_reports", tenant["id"]):
        raise HTTPException(status_code=403, detail="Not authorized to delete reports for this tenant")
    
    # Delete the report
    try:
        report_repository = ReportRepository()
        success = await report_repository.delete_report(report_id, tenant["id"])
        if not success:
